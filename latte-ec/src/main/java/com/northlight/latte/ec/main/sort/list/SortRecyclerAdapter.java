...
import com.northlight.latte.ui.recycler.MultipleViewHolder;
...
public class SortRecyclerAdapter extends MultipleRecyclerAdapter {
    ...
    protected void convert(final MultipleViewHolder holder, final MultipleItemEntity entity) {
        ...
        switch (holder.getItemViewType()){
            case ItemType.VERTICAL_MENU_LIST:
                ...
                final AppCompatTextView tvName = holder.getView(R.id.tv_vertical_item_name);
    ...
    private void showContent(int contentId) {
        final ContentListDelegate delegate = ContentListDelegate.newInstance(contentId);
        switchContent(delegate);
    }

    private void switchContent(ContentListDelegate delegate) {
        ContentListDelegate contentListDelegate =
                SupportHelper.findFragment(DELEGATE.getChildFragmentManager(), ContentListDelegate.class);
        if (contentListDelegate != null){
            contentListDelegate.replaceFragment(delegate,false);
        }
    }
}
